...
import (
	...
	"github.com/crossplane/provider-fitscloudnative/apis"
	"github.com/crossplane/provider-fitscloudnative/apis/v1alpha1"
	fitscloudnative "github.com/crossplane/provider-fitscloudnative/internal/controller"
	"github.com/crossplane/provider-fitscloudnative/internal/features"
	"github.com/crossplane/provider-fitscloudnative/internal/version"
)

func main() {
	var (
		app            = kingpin.New(filepath.Base(os.Args[0]), "FitsCloudnative support for Crossplane.").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-fitscloudnative"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(ratelimiter.LimitRESTConfig(cfg, *maxReconcileRate), ctrl.Options{
		...
		LeaderElectionID:           "crossplane-leader-election-provider-fitscloudnative",
		LeaderElectionResourceLock: resourcelock.LeasesResourceLock,
		LeaseDuration:              func() *time.Duration { d := 60 * time.Second; return &d }(),
		RenewDeadline:              func() *time.Duration { d := 50 * time.Second; return &d }(),
	})
	kingpin.FatalIfError(err, "Cannot create controller manager")
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add FitsCloudnative APIs to scheme")
	...
	if *enableChangeLogs {
		...
		clo := controller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-fitscloudnative:%s", version.Version))),
		}
		o.ChangeLogOptions = &clo
	}

	kingpin.FatalIfError(fitscloudnative.Setup(mgr, o), "Cannot setup FitsCloudnative controllers")
